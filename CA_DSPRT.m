...
L=D-G122;
lambda=eig(L);
delta=2/(lambda(m)+lambda(2)); % minimize sigma 2, largest_lambda + 2nd smallest lambda
W=eye(m)-delta*L;

%% 
disp('loading data...')
load('gaussain_H0_1_12000_5000.mat'); % hypothesis 0, observation at sensor 1
load('gaussain_H1_1_12000_5000.mat');
...
for comb_num=1:length(h_list)
    ...
    [delay_list(comb_num,:)]=CADSPRT_delay(X0,W,m,h0,h1);
    % get erro by change of measure, use H1 instead
    disp(['get error with h=',mat2str(h0)])
    [error_list(comb_num,:)]=CADSPRT_error(X1,W,real_theta,m,h0,h1);
    %
end
