function [ error ] = DualSPRT_error( X, real_theta,h0,h1,gamma0,gamma1,m )
...
Y=zeros(size(L)); % list of secondary sensor triggers
...
for re=1:repeat
    ...
    Yfc(re,:)=sum(Y(m*(re-1)+1:m*re,:));
end
F=2/2*(mu1-mu0)*Yfc+(mu0^2-mu1^2)/2; %log-likelihood ratio again
F=cumsum(F,2); % horizental sum
h1_index=ge(F,h1); % judge if geq b
h0_index=le(F,-h0);
