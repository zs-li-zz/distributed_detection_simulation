function [ error ] = SDDSPRT_error( X,real_theta,m,d,h0,h1)
...
S=sum(L,3);% cumulative log-likelihood ratio
Sc=cumsum(S,2);
...
for re=1:repeat
    if chioce(re)~=real_theta % Means error decision happens. After change of measure, almost all samples will be "wrong"
        ...
        w(re)=w(re)+( 2*delta_miu*sum(sum(X( re, 1:stop(re), : )))-m*stop(re)* delta_miu_square )/(-2*sigma^2);%change measure, log of probability ratio
    end
end
