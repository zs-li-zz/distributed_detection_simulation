...
h_list=1000:1000:8000;
gamma_list=h_list/10;
r_list=[12];
...
for r_index=1:length(r_list)
    ...
    for comb_num=1:length(h_list)
        ...
        gamma0=gamma_list(comb_num); gamma1=gamma0;
        
        % get delay
        disp(['get delay with h=',mat2str(h0)])
        [delay_list(comb_num,:)]=SPRT_CSPRT_delay(X0,h0,h1,gamma0,gamma1,m);
        % get erro by change of measure, use H1 instead
        disp(['get error with h=',mat2str(h0)])
        [error_list(comb_num,:)]=SPRT_CSPRT_error(X1,real_theta,h0,h1,gamma0,gamma1,m);
        %
    end
